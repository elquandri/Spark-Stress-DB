 ...
 */
public class SDBOutputFormat extends FileOutputFormat<Text,Text> {
    ...
    static void setFinalSync(JobContext job, boolean newValue) {
        job.getConfiguration().setBoolean(
                SDBSortConfigKeys.FINAL_SYNC_ATTRIBUTE.key(), newValue);
    }
    ...
    public static boolean getFinalSync(JobContext job) {
        return job.getConfiguration().getBoolean(
                SDBSortConfigKeys.FINAL_SYNC_ATTRIBUTE.key(),
                SDBSortConfigKeys.DEFAULT_FINAL_SYNC_ATTRIBUTE);
    }
    ...
    public void checkOutputSpecs(JobContext job
        ...
        if (fs.exists(outDir)) {
            ...
            if (outDirKids != null && outDirKids.length == 1) {
                ...
                empty =
                        !st.isDirectory() && SDBJInputFormat.PARTITION_FILENAME.equals(fname);
            }
