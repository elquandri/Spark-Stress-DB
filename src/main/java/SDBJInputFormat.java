 ...
 * as Text.
 */
public class SDBJInputFormat extends FileInputFormat<Text,Text> {
    ...
    public static void writePartitionFile(final JobContext job,
        ...
        final SDBJInputFormat inFormat = new SDBJInputFormat();
        final TextSampler sampler = new TextSampler();
        int partitions = job.getNumReduceTasks();
        long sampleSize =
                conf.getLong(SDBSortConfigKeys.SAMPLE_SIZE.key(),
                        SDBSortConfigKeys.DEFAULT_SAMPLE_SIZE);
        final List<InputSplit> splits = inFormat.getSplits(job);
        long t2 = System.currentTimeMillis();
        System.out.println("Computing input splits took " + (t2 - t1) + "ms");
        int samples =
                Math.min(conf.getInt(SDBSortConfigKeys.NUM_PARTITIONS.key(),
                        SDBSortConfigKeys.DEFAULT_NUM_PARTITIONS),
                        splits.size());
    ...
    public List<InputSplit> getSplits(JobContext job) throws IOException {
        ...
        if (job.getConfiguration().getBoolean(SDBSortConfigKeys.USE_TERA_SCHEDULER.key(),
                SDBSortConfigKeys.DEFAULT_USE_TERA_SCHEDULER)) {
            SDBScheduler scheduler = new SDBScheduler(
